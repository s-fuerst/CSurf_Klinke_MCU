...
\subsubsection{Automation}\label{sendautomation}

Since v0.9, send/receive level and pan automation is fully
supported. The automation mode is set independently for each
send. The Automation Mode buttons (\textsc{Read, Write, Trim, Touch,
  Latch}) are still used for the selected track (i.e. the sending or
receiving track). To change the mode for a send, press the corresponding
\rec button. This will toggle between \textsc{Trim} and
\textsc{Touch}. To select one of the other three modes, press and hold
the \rec buttons and press then one of the Automation Mode buttons.
