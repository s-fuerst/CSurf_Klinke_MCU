...
the Dry/Wet level, holding the \alt modifier and pressing \flip
toggles delta solo and holding the \shift modifier and pressing \flip
toggles between 0\% and 100\% Dry/Wet which can be a better glitch free
alternative to enabling/disabling certain plugins if you're attempting
to assess the audible qualities the plugin imparts on the source.


Factory FX maps are not included in the current testing builds. A
future ReaPack distribution is planned and will include the factory maps
again. You can also create your own maps for the selected FX using the
\mapeditor (see section~\ref{createmaps} for details). If no map exists
for an FX, a mapping is created automatically. In this case parameters
that are detected as discrete (e.g.\ on/off switches or waveform
selectors) are mapped to the \vpots, all other parameters to the
\faders; parameters that lie next to each other in the parameter
list of the FX share a channel strip (so e.g.\ an LFO waveform and
the LFO rate that follows it are controlled by the \vpot and the
\fader of one channel). If no discrete values can be detected for
the FX, all parameters are mapped to the \faders as before.
...
via \alt \plug (see Figure~\ref{F:Screenshot_Plug_Mode}). 

\pikFigure[0.54][9.3cm][-2.3mm][0.2mm]{Screenshot_Plug_Mode}{Mapping
  editor (\alt \plug)}{}{}

Parameters can be assigned to the strips of a page with the Learn
function of the mapping editor: enable the ``Learn'' checkbox at the
top of the editor, select the \fader or \vpot of the channel in the
editor (the element that was clicked last is used, its parameter
combo box is shown in red) and change the desired value in the FX
user interface. The parameter that changes is assigned to the
selected element. Alternatively you can pick the parameter directly
from the combo box next to the element.

If a parameter is assigned to a \vpot -- with the Learn option or by
selecting it in the parameter list -- the value table of the \vpot
is filled automatically. If the FX reports the discrete values of
the parameter (e.g.\ for the waveform selector of an oscillator),
these values and their names are used, otherwise they are detected
with a heuristic. If no discrete values can be detected, the table
stays empty and the values can be learned manually by setting the
parameter to the missing value while Learn is enabled. The short and
long names in the table can be edited afterwards; the automatically
created short names use a heuristic that keeps them distinguishable
on the display.
...
section). Factory maps are currently absent from the testing downloads;
see section~\ref{installation} for the planned ReaPack distribution.
So if you edit a map and then select a different FX, these changes are lost. When you start to work on a map, it is a good idea
...
select another track, the FX that was selected in this track gets the
focus again. By pressing the \gv button (its LED lights up) this
connection between tracks and the controlled FX is broken: the
controlled FX stays fixed until a different FX is selected manually.

In Favorites mode the \select buttons act as favorite slots instead of
FX selectors. The LEDs show the slot state: the slot that holds the
currently accessed FX blinks, a slot that holds another stored
favorite is lit solid (blinking if that FX is bypassed), and an empty
slot is dark. Favorites are stored in the \reaper project file.

In Favorites mode the \select buttons behave as follows:

\begin{compactitem}
\item \select: Recall the favorite stored in this slot.
\item \control: Store the currently accessed FX in this
  slot 
\item \option: Clear the favorite stored in this slot.
\item \shift: Address slots 9-16, so you can store up to 16
  favorites.
\end{compactitem}

An FX that is not (yet) a favorite can only be selected via the
\vpots, see the section above.
...
\item \select: Select an FX of the actual track (if the \gv is off) or a
  ...
  \bemod
  \begin{compactitem}
  \item \control: If Favorites mode is active: store the currently
    accessed FX as a Favorite.
  \item \option: If Favorites mode is active: clear the Favorite in
    this slot.
...
\item \gv: see \select
\item \flip: Bypass
\item \shift \flip: Toggle 0/100\% Dry/Wet level.
\item \alt \flip: Delta Solo
...
In FX-Mode the meterbridge displays the level of the first eight
channels of the selected track, unless the device has like the Mackie Control Universal only a virtual 
meterbridge (on the display).

\subsubsection{Commands}\label{plugcommands}
In the FX-Mode the \hyperref[commands]{Commands} act on the accessed plugin.
Press the \vpot while holding the \control modifier; while \control is held,
the display shows the command label on the \vpots:

\begin{itemize}
\item \control + \vpot 1 (\emph{Float}): Toggle the floating window of the
  accessed plugin. This command is not offered while the ``Limit float''
  \attribute is set to ``only chain''.
\item \control + \vpot 2 (\emph{Chain}): Toggle the FX chain window of the
  track of the accessed plugin.
\item \control + \vpot 5 (\emph{Remove}): Remove the accessed plugin from the
  track.
\item \control + \vpot 7 / \vpot 8 (\emph{FXup} / \emph{FXdown}): Move the
  accessed plugin one slot up / down in the FX chain.
\end{itemize}
