...
\item \mcu runs on Windows, macOS\,(11 Big Sur or newer) and Linux.
  The macOS build currently supports Apple Silicon (arm64) only. The
  source code is open and is based on the cross-platform JUCE library.

\item The minimum supported \reaper version is 6.37.

\item The Linux build is for 64-bit x86 systems and requires glibc 2.31 or
  newer (for example Ubuntu 20.04+ or Debian 11+). A working X11 desktop,
  FreeType, Fontconfig and libcurl installation is required for the REAPER
  GUI and the extension. Wayland-only setups may need an X11 compatibility
  session.
\end{itemize}
...
\begin{compactitem}
\item Cross-platform support: the extension now runs on Windows, macOS
  and Linux.
