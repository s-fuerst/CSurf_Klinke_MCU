...
\item \rec: Arm Track on/off
        ...
        \item \shift: Monitor on/off. As long as \shift is pressed the
          ...
          is active). This works independently of the track being
          record-armed.
        \item \option: Rec Mode Input/None. As long as \option is
          pressed the \rec button LEDs show the Rec Mode status
          (On = Rec Mode is set to Input, Off = all other settings).
          This works independently of the track being record-armed.
        \item \alt: Rec Mode Output/None. As long as \alt is pressed
          the \rec button LEDs show the Rec Mode status (On = Rec
          Mode is set to Output (Stereo or Mono), Off = all other
          settings). The \rec button toggles between the three
          states: None/Output Stereo/Output Mono. This works
          independently of the track being record-armed.
        \end{itemize}
...
select an anchor slot (one per channel strip) in the Anchor column of the
...
\subsubsection{Commands}\label{pancommands}
In the Pan-Mode the \hyperref[commands]{Commands} act on the track that is
shown on the pressed channel. Press the \vpot while holding the \control
modifier; while \control is held, the display shows the command label on the
\vpots (an empty channel can only insert a track):

\begin{itemize}
\item \control + \vpot 1 (\emph{Insert}): Insert a new track directly after
  the track on this channel. On an empty channel the new track is inserted at
  the beginning of the track list.
\item \control + \vpot 2 (\emph{Duplic}): Duplicate the track, including its
  items and envelopes, inserted after the original.
\item \control + \vpot 3 (\emph{Clear}): Delete every item and every envelope
  of the track.
\item \control + \vpot 4 (\emph{Remove}): Remove the track.
\end{itemize}
