...
public:
  SendReceiveMeterBridge(SendReceiveModeBase *pSendMode);
	// The emulated LCD meter is disabled in Send/Receive mode: the track
	// names are more important to read on the display than the level
	// segments. The hardware meter LEDs (strip meters via 0xD0 and ProX
	// master meters) still work — they are driven regardless of this flag.
